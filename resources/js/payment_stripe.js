...
function mf_submit_payment(){
	...
	var mf_number = null;
	var mf_cvc = null;
	var mf_exp_month = null;
	var mf_exp_year = null;

	//billing address
	var mf_address_line1 = null;
	var mf_address_city = null;
	var mf_address_state = null;
	var mf_address_zip = null;
	var mf_address_country = null;
	...
	Stripe.createToken({
		number: mf_number,
		cvc: mf_cvc,
		exp_month: mf_exp_month,
		exp_year: mf_exp_year,
		name: mf_name,
		address_line1: mf_address_line1,
		address_city: mf_address_city,
		address_state: mf_address_state,
		address_zip: mf_address_zip,
		address_country: mf_address_country
	}, mf_stripe_response_handler);
	
}

//callback for stripe's create token
//this is the main function to actually charge the card
function mf_stripe_response_handler(status, response) {
	if(response.error) {	
